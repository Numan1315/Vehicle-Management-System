...
def insertdealer():
    if request.method == 'POST':
        ...
        name = obj['name']
        cno = obj['cno']
        cur.execute("insert into showroom_owner values({0},'{1}',{2})".format(did,name,cno))
        con.commit()
...
def viewdealer():
    cur.execute("select * from showroom_owner")
    rows = cur.fetchall()
    col = [desc[0] for desc in cur.description]
    return render_template('view.html',content = [rows,col])
...
def selectdealer():
    if request.method == 'POST':
        ...
        cur.execute("select * from showroom_owner where d_id = {0}".format(did))
        rows = cur.fetchall()
        col = [desc[0] for desc in cur.description]
        return render_template('view.html',content = [rows,col])
    else:
